#!/usr/bin/env python3
"""
运行脚本：依次处理5个ray_tracing_results并保存结果 (FKB版本)

使用方法:
    python run_all_scenarios_fkb.py

或者单独运行某个场景:
    python run_all_scenarios_fkb.py --scenario TJU_north
"""
...
def run_scenario(scenario_name, num_users=NUM_USERS):
    ...
    # 读取源文件并替换路径
    source_file = os.path.join(project_root, "with_knowledge_base", "WA_DS_V3_FKB.py")
    ...
    try:
        ...
        import with_knowledge_base.WA_DS_V3_FKB as wa_module
        importlib.reload(wa_module)
        WA_DS_V3_FKB = wa_module

        # 清空全局状态
        WA_DS_V3_FKB.reset_token_stats()
        WA_DS_V3_FKB.reset_network_state()

        WA_DS_V3_FKB.main(num_users=num_users, export_file=scenario["export_file"])
...
def main():
    parser = argparse.ArgumentParser(description="运行网络切片场景 (FKB版本)")
    parser.add_argument(
        "--scenario",
        type=str,
