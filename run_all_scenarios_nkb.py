#!/usr/bin/env python3
"""
运行脚本：依次处理5个ray_tracing_results并保存结果

使用方法:
    python run_all_scenarios_nkb.py

或者单独运行某个场景:
    python run_all_scenarios_nkb.py --scenario TJU_north
"""
...
def run_scenario(scenario_name, num_users=NUM_USERS):
    ...
    # 读取源文件并替换路径
    source_file = os.path.join(project_root, "no_knowledge_base", "WA_DS_V3_NKB.py")
    ...
    try:
        ...
        import no_knowledge_base.WA_DS_V3_NKB as wa_module
        importlib.reload(wa_module)
        WA_DS_V3_NKB = wa_module

        # 清空全局状态
        WA_DS_V3_NKB.reset_token_stats()
        WA_DS_V3_NKB.reset_network_state()

        WA_DS_V3_NKB.main(num_users=num_users, export_file=scenario["export_file"])
...
def main():
    parser = argparse.ArgumentParser(description="运行网络切片场景")
    parser.add_argument(
        "--scenario",
        type=str,
