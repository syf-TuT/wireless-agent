#!/usr/bin/env python3
"""
Batch runner: execute all ray tracing scenarios for FKB mode.
Usage:
  python run_all_scenarios_fkb.py
  python run_all_scenarios_fkb.py --scenario TJU_south --num-users 30
"""
...
MODEL_NAME = "glm-4.7"
OUTPUT_DIR = r"F:\code\wirelessagent\run_results\batch_run\fkb\glm-4.7"
...
def run_scenario(scenario_name: str, num_users: int = NUM_USERS) -> None:
    ...
    source_file = os.path.join(project_root, "with_knowledge_base", "WA_FKB.py")
    ...
    try:
        import with_knowledge_base.WA_FKB as wa_module
...
def run_all_scenarios(num_users: int = NUM_USERS) -> None:
    print("=" * 60)
    print(f"Running all scenarios (model: {MODEL_NAME})")
    print("=" * 60)

    for i, scenario_name in enumerate(SCENARIOS.keys(), 1):
        print(f"\n{'=' * 60}")
        print(f"Scenario {i}/{len(SCENARIOS)}: {scenario_name}")
        print(f"Input : {SCENARIOS[scenario_name]['ray_tracing_csv']}")
        print(f"Output: {SCENARIOS[scenario_name]['export_file']}")
        print("-" * 60)
        run_scenario(scenario_name, num_users)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run network slicing scenarios (FKB)")
