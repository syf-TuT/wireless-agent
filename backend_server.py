...
KNOWLEDGE_BASE_DIR = os.path.join(PROJECT_ROOT, "with_knowledge_base")


def get_wirelessagent_module():
    """Load the knowledge-base WirelessAgent module lazily."""
    for path in (PROJECT_ROOT, KNOWLEDGE_BASE_DIR):
        if path not in sys.path:
            sys.path.insert(0, path)
    return importlib.import_module("with_knowledge_base.WA_KB")
...
async def get_results():
    try:
        result_file = "with_knowledge_base/network_slicing_results_KB.csv"
...
async def get_network_state():
    ...
    def get_state_sync():
        wirelessagent = get_wirelessagent_module()
        return wirelessagent.get_current_network_state()
